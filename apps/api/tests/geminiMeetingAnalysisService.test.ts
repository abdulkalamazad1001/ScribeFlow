...
describe("GeminiMeetingAnalysisService", () => {
  ...
  it("retries transient provider failures with backoff and then succeeds", async () => {
    ...
    const generateContent = vi
      .fn()
      .mockRejectedValueOnce(new Error("provider request failed"))
      .mockRejectedValueOnce(new Error("provider request failed again"))
      .mockResolvedValueOnce({
        text: JSON.stringify(validAnalysis),
        responseId: "recovered-after-retry",
      });
    ...
    expect(result.responseId).toBe("recovered-after-retry");
  ...
  it("does not retry non-transient auth failures", async () => {
    ...
    ).rejects.toMatchObject({ code: "GEMINI_AUTH_FAILED" });
