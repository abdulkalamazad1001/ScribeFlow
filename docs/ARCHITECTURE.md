...
Real-time live transcription is implemented. The browser streams microphone
audio over a WebSocket to the API, which relays it to Deepgram's streaming
endpoint and pushes partial/final transcripts back live. On stop, the API
persists the final transcript and reuses the uploaded-audio analysis + indexing
pipeline.
...
sequenceDiagram
  ...
  Browser->>API: Open WebSocket /api/meetings/live/socket?meetingId
  API->>Database: Mark meeting transcribing
  API->>Deepgram: Open streaming session (diarize, interim_results)
  Browser->>API: Stream microphone audio frames (WebM/Opus)
  API->>Deepgram: Forward audio frames
  Deepgram-->>API: Partial and final transcript events
  API-->>Browser: Live transcript updates (interim + final)
  Browser->>API: Stop message
  API->>Deepgram: CloseStream (flush remaining finals)
  Deepgram-->>API: Final transcripts, then close
  API->>Database: replace_meeting_transcription (segments, speakers)
  API->>Database: Persist Gemini analysis + RAG index
  API-->>Browser: Completed (meetingId)
```

Implementation notes:

- The browser never receives the Deepgram key; audio flows browser → API →
  Deepgram. For live (unlike uploads) audio passes through the API process.
- The API connects to Deepgram's streaming endpoint with a raw WebSocket
  (`deepgramLiveTranscriptionService.ts`); the relay lives in
  `realtime/liveMeetingSocket.ts`.
- Each Deepgram final ("Results") message becomes a transcript segment,
  preserving natural utterance boundaries and splitting on speaker change.
- Finalisation waits for Deepgram's upstream close (all finals flushed) with a
  safety timeout, so trailing finals are not lost when audio streams faster than
  real time.
- Live diarisation is less precise than batch diarisation for uploaded audio;
  speaker labels may be coarser.
