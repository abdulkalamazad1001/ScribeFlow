...
export const buildLiveSocketUrl = (meetingId: string) => {
  const httpUrl = buildApiUrl(
    `/meetings/live/socket?meetingId=${encodeURIComponent(meetingId)}`,
  );
  const origin =
    typeof window !== "undefined" ? window.location.origin : "http://localhost";
  const absolute = httpUrl.startsWith("http") ? httpUrl : `${origin}${httpUrl}`;
