...
const retryBaseBackoffMs = 500;
const retryMaxBackoffMs = 8_000;
...
// Transient provider failures worth retrying. Auth failures and schema/output
// validation errors are deliberately excluded — retrying those just repeats the
// same failure.
const retryableGeminiErrorCodes = new Set([
  "GEMINI_REQUEST_FAILED",
  "GEMINI_RATE_LIMITED",
  "GEMINI_REQUEST_TIMEOUT",
]);

const isRetryableTransientGeminiError = (error: ApiError) =>
  retryableGeminiErrorCodes.has(error.code);

const getRetryBackoffMs = (attempt: number) =>
  Math.min(retryMaxBackoffMs, retryBaseBackoffMs * 2 ** attempt);
...
// One schema-constrained attempt, with a JSON-mode fallback for long meetings
// whose schema-constrained generation fails.
async function generateGeminiContentWithSchemaFallback(input: {
  client: GeminiAnalysisClient;
...
// Retry the whole schema-fallback sequence on transient provider failures
// (network blips, 5xx, rate limits, timeouts) with exponential backoff. Long
// meetings produce large requests that fail transiently more often, so a
// bounded retry prevents a single hiccup from failing the whole analysis.
async function generateGeminiContentWithLongMeetingFallback(input: {
  ...
  for (let attempt = 0; ; attempt += 1) {
    try {
      return await generateGeminiContentWithSchemaFallback(input);
    } catch (error) {
      const mapped = mapGeminiError(error);
      if (attempt >= maxRetries || !isRetryableTransientGeminiError(mapped)) {
        throw mapped;
      }
      ...
      logger.warn(
        ...
        "transient gemini error; retrying after backoff",
      );
